...
export default function Dashboard() {
  ...
  async function handleAddMovie(e: React.FormEvent) {
    ...
    try {
      ...
      setTitle(""); setWatchedDate(""); setRating(""); setGenres(""); setTags(""); setDirector(""); setActors(""); setNotes(""); setPosterUrl("");
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <button className="px-4 py-2 bg-zinc-300 rounded hover:bg-zinc-400" onClick={handleLogout}>Logout</button>
      </div>
      ...
      <Card className="p-6 mb-8">
        ...
        <form className="space-y-3" onSubmit={handleAddMovie}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label>Date Watched</Label><Input type="date" value={watchedDate} onChange={e => setWatchedDate(e.target.value)} required/></div>
            <div><Label>Rating</Label><Input type="number" min={1} max={10} value={rating} onChange={e => setRating(e.target.value)} required/></div>
            <div><Label>Genres <span className="text-xs">(comma separated)</span></Label><Input value={genres} onChange={e => setGenres(e.target.value)} required/></div>
            <div><Label>Tags <span className="text-xs">(optional, comma separated)</span></Label><Input value={tags} onChange={e => setTags(e.target.value)} /></div>
            <div><Label>Director</Label><Input value={director} onChange={e => setDirector(e.target.value)} required/></div>
            <div><Label>Actors <span className="text-xs">(comma separated)</span></Label><Input value={actors} onChange={e => setActors(e.target.value)} required/></div>
            <div><Label>Poster URL <span className="text-xs">(optional)</span></Label><Input value={posterUrl} onChange={e => setPosterUrl(e.target.value)}/></div>
          </div>
          <div><Label>Notes <span className="text-xs">(optional)</span></Label><Input value={notes} onChange={e => setNotes(e.target.value)} /></div>
          <Button type="submit" disabled={submitting}>{submitting ? "Adding..." : "Add Movie"}</Button>
              ...
              <div className="flex-1">
                {editingId === mov._id ? (
                  <form className="grid gap-2" onSubmit={e => submitEdit(e, mov._id)}>
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <Input type="date" value={editFields.watchedDate} onChange={e => updateEditField("watchedDate", e.target.value)} required />
                      </div>
                      <div>
                        <Label>Rating</Label>
                        <Input type="number" min={1} max={10} value={editFields.rating} onChange={e => updateEditField("rating", e.target.value)} required />
                      </div>
                      <div>
                        <Label>Genres</Label>
                  ...
                  <>
                    ...
                    <p className="text-sm text-zinc-600">Watched {mov.watchedDate ? new Date(mov.watchedDate).toLocaleDateString() : ""}</p>
                    <p>🎬 {mov.director} | ⭐ {mov.rating}/10</p>
                    <div><span className="font-semibold">Genres:</span> {mov.genres.join(", ")}</div>
                    {mov.tags && mov.tags.length > 0 && <div><span className="font-semibold">Tags:</span> {mov.tags.join(", ")}</div>}
                    <div><span className="font-semibold">Actors:</span> {mov.actors.join(", ")}</div>
                    {mov.notes && <div><span className="font-semibold">Notes:</span> {mov.notes}</div>}
